...
sed "s|K8S-APISERVER-ADDR|${node_ip}:8443|" "${dir}/spark-defaults.conf.template" | \
  sed "s|NAMESPACE|${namespace}|" > "${dir}/spark-defaults.conf"
...
sed "s|JUICEFS_JAR_LOCATION|${juicefs_dir}|" "${dir}/kyuubi.yaml" | \
  sed "s|JUICEFS_JAR_NAME|${juicefs_jar}|" > "${dir}/kyuubi.yaml.tmp"
