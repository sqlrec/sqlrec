...
Local `UNION ALL` and `UNION DISTINCT` are both implemented through `SqlrecEnumerableUnion` using a snake merge algorithm.

#### Implementation

```java
// SqlrecEnumerableUnion.implement
// comparerAndInputExps = comparer followed by every inputExp
...
Snake merge polls the input sources and alternates one row from each, then materializes all rows into a `List`. It is not lazy streaming output. `UNION DISTINCT` deduplicates with the `EqualityComparer` generated by Calcite for the current physical row type; a duplicate does not consume that source's accepted-output quota for the round.
...
`UNION ALL` preserves duplicates, while `UNION` (`UNION DISTINCT`) deduplicates complete rows. Snake merge determines the round-robin order normally observed from the current implementation, but SQL does not guarantee the order of an unsorted result; add an outermost `ORDER BY` when stable ordering is required.

#### UNION Branch Failure Recovery

When a SQL function is compiled, SQLRec identifies degradable branches from real table read/write dependencies. At execution time it evaluates `IGNORE_UNION_EXCEPTION` (default `true`). A cache-table producer is recoverable only when all of the following are true:

- the node eventually feeds `UNION`/`UNION ALL`, or a statically bound table function that implements `UnionLikeTableFunction` (the built-in implementation is `weighted_merge`);
- every data-consumer chain from that node eventually enters such a merge, with no path ending in a return before entering a merge, standalone execution, or another non-merge sink;
- the failed node exposes a cache-table name and field schema, allowing an empty `CacheTable` with the same schema to replace it.

For an eligible node, an ordinary failure or node timeout replaces that branch with an empty table and lets the remaining branches merge. Cancellation, thread interruption, and `Error` are not swallowed. Unused ordinary nodes, producers returned without first entering a merge, and shared inputs that also flow to a non-merge path are not degraded. `SET IGNORE_UNION_EXCEPTION=false` disables recovery in the current execution context without changing the compile-time dependency graph. A function name resolved dynamically through `GET()` has no statically bound instance, so `UnionLikeTableFunction` cannot be detected at compile time and the call is not classified as a merge node.
...
| UNION ALL / UNION DISTINCT | Data merging / complete-row distinct merge | `SqlrecEnumerableUnion` + `MergeUtils` snake merge algorithm |
