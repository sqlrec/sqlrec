...
**Layer A — Path resolution** (``url_to_fs``, ``resolve_path``):
    ...
    are resolved through fsspec, which manages filesystem instance caching.
...
def url_to_fs(path: str) -> tuple[fsspec.AbstractFileSystem | None, str]:
    ...
    fsspec-supported protocol), fsspec resolves the filesystem for that URL.
    """
...
def download_dir(remote_dir: str, local_dir: str) -> None:
    ...
    if fs is not None:
        base_path = fs._strip_protocol(full)
        for entry in fs.find(base_path):
            if not fs.isfile(entry):
                continue
            rel = os.path.relpath(fs._strip_protocol(entry), base_path)
            if rel == ".." or rel.startswith("../"):
                raise ValueError(f"File outside model directory: {entry}")
...
def copy_file(src_path: str, dst_path: str) -> None:
    ...
    # Stream copy.
    open_src = src_fs.open if src_fs is not None else _original_open
    open_dst = dst_fs.open if dst_fs is not None else _original_open
    with open_src(src_full, "rb") as src, open_dst(dst_full, "wb") as dst:
        shutil.copyfileobj(src, dst)
...
def _patched_copy(src, dst, *args, **kwargs):
    ...
    if src_fs is not None or dst_fs is not None:
        src_fs = src_fs or fsspec.filesystem("file")
        dst_fs = dst_fs or fsspec.filesystem("file")
...
def _patched_glob(pattern, *args, **kwargs):
    ...
    if fs is not None:
        native_pattern = fs._strip_protocol(full)
        suffix = native_pattern.lstrip("/")
        prefix = full[:-len(suffix)] if suffix else full
        return [path if "://" in path else prefix + path.lstrip("/")
                for path in fs.glob(native_pattern, *args, **kwargs)]
